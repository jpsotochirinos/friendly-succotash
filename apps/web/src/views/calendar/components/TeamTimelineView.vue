<template>
  <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
    <p
      v-if="users.length === 0"
      class="m-0 px-1 py-4 text-center text-sm text-[var(--fg-muted)]"
    >
      {{ t('globalCalendar.teamTimelineNoColumns') }}
    </p>
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div class="min-h-0 flex-1 overflow-y-auto overflow-x-auto overscroll-contain">
        <div class="inline-flex min-w-full min-h-min">
          <!-- Time gutter -->
          <div
          ...
          >
            <div
              ...
              :key="h"
              class="border-b border-[var(--surface-border)]/60 px-1 text-[10px] text-[var(--fg-muted)]"
              :style="{ height: `${PX}px` }"
            >
          ...
          >
            ...
            >
              <span class="text-xs font-semibold text-[var(--fg-default)] truncate max-w-full" :title="u.label">{{
                u.label
              }}</span>
