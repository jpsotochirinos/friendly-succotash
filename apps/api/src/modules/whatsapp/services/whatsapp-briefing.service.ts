...
export class WhatsAppBriefingService {
  private readonly logger = new Logger(WhatsAppBriefingService.name);

  constructor(private readonly em: EntityManager) {}

  async buildGroupBriefing(organizationId: string): Promise<string> {
  ...
  async buildPersonalBriefing(organizationId: string, userId: string): Promise<string> {
    const conn = this.em.getConnection();
